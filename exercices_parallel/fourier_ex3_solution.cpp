...
int main(int argc, char **argv)
{
  ...
  int BoxSize=64;
  int renormFFT = BoxSize*BoxSize*BoxSize;
  Lattice lat(3,BoxSize,1);
  Lattice latK;
  ...
  laplace_phi.initialize(lat,comp);
  Field<Imag> laplace_phiK;
  laplace_phiK.initialize(latK,comp);
  PlanFFT<Imag> planPhi(&laplace_phi,&laplace_phiK);
  ...
  for(x.first();x.test();x.next())
  {
      laplace_phi(x) =  (phi(x+0) - 2 * phi(x) + phi(x-0))/res2;
      laplace_phi(x) += (phi(x+1) - 2 * phi(x) + phi(x-1))/res2;
      laplace_phi(x) += (phi(x+2) - 2 * phi(x) + phi(x-2))/res2;
  }
  ...
  for(;k.test();k.next())
  {
      laplace_phiK(k) /=  ( 2.0 *(cos(2.0*M_PI*k.coord(0)/BoxSize)
  ...
  for(x.first();x.test();x.next())
  {
      error =  fabs( phi(x) - (laplace_phi(x)/renormFFT) ) ;
      averageError+=error;
